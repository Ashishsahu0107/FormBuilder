  ...
  async (req: AuthRequest, res: Response) => {
    try {
      ...
      if (!version || version.status !== "UNDER_REVIEW")
        return sendError(res, "Form is not under review", 400);
...
router.post(
  ...
  async (req: AuthRequest, res: Response) => {
    try {
      if (!req.body.comment)
        return sendError(res, "Comment is required for rejection", 400);
  ...
  async (req: AuthRequest, res: Response) => {
    try {
      ...
      if (!version || version.status !== "APPROVED")
        return sendError(res, "Only APPROVED versions can be published", 400);
...
router.post(
  ...
  async (req: AuthRequest, res: Response) => {
    try {
      const form = await Form.findOne({ _id: req.params.id, deletedAt: null });
      if (!form) return sendError(res, "Form not found", 404);
      if (form.status !== "PUBLISHED" && form.status !== "DEACTIVATED")
        return sendError(res, "Invalid status for activation", 400);
      ...
      return sendSuccess(
        res,
        null,
        "Form is now active and ready for submissions",
      );
    } catch (error) {
  ...
  async (req: AuthRequest, res: Response) => {
    try {
      ...
      if (form.status !== "ACTIVE")
        return sendError(res, "Only ACTIVE forms can be deactivated", 400);
