...
function renderElementContent(element: CanvasElement, isEditing: boolean, editRef: React.RefObject<HTMLDivElement | null>) {
  ...
  if (element.type === 'heading' || element.type === 'subheading' || element.type === 'paragraph') {
    ...
    return <div style={{ ...textStyle, pointerEvents: 'none' }}>{element.content}</div>
  }

  if (element.type === 'field_divider') {
    return <div style={{ width: '100%', height: '1px', backgroundColor: s.borderColor || '#d1d5db' }} />
  }

  if (element.type === 'field_checkbox') {
    return (
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, ...textStyle, height: '100%' }}>
        <div style={{
          width: 16, height: 16, border: `1.5px solid ${s.borderColor || '#6b7280'}`,
          borderRadius: 3, flexShrink: 0, backgroundColor: 'white'
        }} />
        <span style={{ fontSize: s.fontSize, color: s.color }}>{element.content}</span>
      </div>
    )
  }
  ...
  if (element.type === 'field_signature') {
    return (
      <div style={{ width: '100%', height: '100%', display: 'flex', flexDirection: 'column', justifyContent: 'flex-end' }}>
        <div style={{ fontSize: s.fontSize - 2, color: '#9ca3af', marginBottom: 4 }}>{element.label || element.content}</div>
        <div style={{ borderBottom: `1.5px solid ${s.borderColor || '#374151'}`, width: '100%', height: '60%' }} />
      </div>
    )
  }
  ...
  if (element.type === 'field_image_placeholder') {
    return (
      <div style={{
        ...
        color: '#9ca3af', fontSize: 12, flexDirection: 'column', gap: 4
      }}>
  ...
  if (element.type === 'field_dropdown') {
    return (
      <div style={{ width: '100%', height: '100%', display: 'flex', flexDirection: 'column', justifyContent: 'flex-end', gap: 4 }}>
        <label style={{ fontSize: s.fontSize - 2, color: '#374151', fontWeight: 'bold' }}>{element.label || element.content}</label>
        <div style={{
          width: '100%',
          ...
          fontSize: s.fontSize - 1, color: '#9ca3af'
        }}>
          <span>Select...</span>
  ...
  const isTextArea = element.type === 'field_textarea' || element.type === 'field_address'
  return (
    <div style={{ width: '100%', height: '100%', display: 'flex', flexDirection: 'column', justifyContent: 'flex-end', gap: 4 }}>
      <label style={{ fontSize: s.fontSize - 2, color: '#374151', fontWeight: 'bold' }}>{element.label || element.content}</label>
      <div style={{
...
export function CanvasElementComponent({ element, isSelected, scale, onSelect, editor }: CanvasElementProps) {
  const [isDragging, setIsDragging] = useState(false)
  const [isResizing, setIsResizing] = useState(false)
  const [isEditing, setIsEditing] = useState(false)
  const dragStartRef = useRef({ mouseX: 0, mouseY: 0, elX: 0, elY: 0 })
  const resizeStartRef = useRef({ mouseX: 0, mouseY: 0, elX: 0, elY: 0, elW: 0, elH: 0, dir: 'se' as ResizeDirection })
  const editRef = useRef<HTMLDivElement>(null)

  const isText = element.type === 'heading' || element.type === 'subheading' || element.type === 'paragraph'
  ...
  useEffect(() => {
    ...
    const handleMouseMove = (e: MouseEvent) => {
      if (isDragging) {
        ...
        editor.moveElement(element.id, dragStartRef.current.elX + dx, dragStartRef.current.elY + dy)
      }
      if (isResizing) {
        const { mouseX, mouseY, elX, elY, elW, elH, dir } = resizeStartRef.current
        ...
        if (dir.includes('w')) { nx = elX + dx; nw = elW - dx }
        if (dir.includes('n')) { ny = elY + dy; nh = elH - dy }
    ...
    const handleMouseUp = (e: MouseEvent) => {
      if (isDragging) {
        const dx = (e.clientX - dragStartRef.current.mouseX) / scale
        const dy = (e.clientY - dragStartRef.current.mouseY) / scale
        editor.commitMove(element.id, dragStartRef.current.elX + dx, dragStartRef.current.elY + dy)
        setIsDragging(false)
      }
      if (isResizing) {
        const { mouseX, mouseY, elX, elY, elW, elH, dir } = resizeStartRef.current
  ...
  return (
    ...
    >
      {renderElementContent(element, isEditing, editRef)}

      {isEditing && isText && (
        <div
          style={{ position: 'fixed', inset: 0, zIndex: -1 }}
          onMouseDown={(e) => { e.stopPropagation(); handleEditBlur() }}
        />
      )}
