...
export function ElementLibrary({ editor }: ElementLibraryProps) {
  return (
    <div className="flex flex-col h-full bg-white border-r border-gray-200" style={{ width: 220, minWidth: 220 }}>
      ...
      <div className="flex-1 overflow-y-auto p-3 space-y-4">
        {FIELD_GROUPS.map(group => {
          ...
          return (
            <div key={group}>
              <div className="text-xs font-bold text-gray-400 uppercase tracking-wider mb-2 px-1">{group}</div>
              <div className="grid grid-cols-2 gap-2">
                {items.map(item => (
                  <button
                    key={item.type}
                    onClick={() => editor.addElement(item)}
                    className="flex flex-col items-center gap-1.5 p-2 rounded-lg border border-gray-200 hover:border-blue-400 hover:bg-blue-50 transition-all group text-center"
                    title={item.label}
                  >
