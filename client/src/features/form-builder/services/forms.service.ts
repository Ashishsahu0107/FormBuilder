...
export const formsService = {
  ...
  getVersions: (formId: string) =>
    api.get<{ success: boolean; data: FormVersion[] }>(
  ...
  createVersion: (formId: string) =>
    api.post<{ success: boolean; data: FormVersion }>(
      `/forms/${formId}/versions`,
    ),

  saveSchema: (formId: string, versionId: string, schema: FormSchema) =>
    api.patch<{ success: boolean; data: FormVersion }>(
      `/forms/${formId}/versions/${versionId}`,
      { schema },
    ),
...
export const publicFormsService = {
  getForm: (slug: string) =>
    api.get<{
      success: boolean;
      data: { schema: FormSchema; title: string; description?: string };
    }>(`/public/forms/${slug}`),
