...
export function DeleteFormModal({
  ...
  if (targetText.length > 0) {
    ...
    matchPercentage = Math.min(
      100,
      Math.round((matches / targetText.length) * 100),
    );
  }
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              <label className="text-sm font-semibold text-gray-700">
                Type{" "}
                <span className="font-mono bg-gray-100 text-red-600 px-1.5 py-0.5 rounded border border-gray-200">
                  {form?.title}
                </span>{" "}
                to confirm
              </label>
                ...
                className={`font-mono text-sm h-12 transition-all duration-300 pr-10 ${
                  isMatch
                    ...
                    : deleteConfirmText.length > 0 &&
                        matchPercentage < 100 &&
                        deleteConfirmText !==
                          targetText.substring(0, deleteConfirmText.length)
