      ...
      <DialogContent className="max-w-4xl max-h-[90vh] flex flex-col p-0">
        ...
        <div className="flex-1 overflow-y-auto p-6 bg-gray-50 space-y-6">
          {logic.map((rule, rIdx) => (
            <div
              key={rule.id}
              className="bg-white border border-gray-200 rounded-lg p-5 shadow-sm space-y-4 relative"
            >
                    ...
                    <div key={cIdx} className="flex gap-2 items-center">
                      <select
                        ...
                        onChange={(e) => {
                          ...
                          newC[cIdx].fieldId = e.target.value;
                      ...
                      <Button
                        ...
                        size="icon"
                        onClick={() => {
                          const newC = rule.conditions.filter(
                            (_, i) => i !== cIdx,
                          );
                          updateRule(rIdx, { conditions: newC });
                        }}
                  ...
                  {rule.actions.map((act, aIdx) => (
                    <div key={aIdx} className="flex gap-2 items-center">
                      <select
                        ...
                        value={act.type}
                        onChange={(e) => {
                          const newA = [...rule.actions];
                          newA[aIdx].type = e.target.value as any;
                      ...
                      <select
                        ...
                        onChange={(e) => {
                          const newA = [...rule.actions];
                          newA[aIdx].fieldId = e.target.value;
                          updateRule(rIdx, { actions: newA });
                        }}
                      >
                        <option value="">Select Field...</option>
                      ...
                      {act.type === "SET_VALUE" && (
                        <Input
                          ...
                          onChange={(e) => {
                            const newA = [...rule.actions];
                            newA[aIdx].value = e.target.value;
                            updateRule(rIdx, { actions: newA });
                          }}
                        />
                      )}
                      ...
                      <Button
                        ...
                        size="icon"
                        onClick={() => {
                          const newA = rule.actions.filter(
                            (_, i) => i !== aIdx,
                          );
                          updateRule(rIdx, { actions: newA });
                        }}
