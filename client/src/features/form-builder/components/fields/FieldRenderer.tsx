...
export function FieldRenderer({
  ...
  switch (field.type) {
    ...
    case "radio":
      return (
        <div className="space-y-1">
          ...
          <div
            className={`flex ${field.config?.layout === "horizontal" ? "flex-row gap-4" : "flex-col gap-2"}`}
          >
            {(field.options || []).map((opt) => (
              <label
                key={opt.value}
          ...
          <div className="border-2 border-dashed border-gray-300 rounded-md p-6 text-center">
            ...
            {!!field.config?.acceptedTypes && (
              <p className="text-xs text-gray-400 mt-1">
                {(field.config.acceptedTypes as string[]).join(", ")}
              </p>
            )}
            {!!field.config?.maxSize && (
              <p className="text-xs text-gray-400">
                Max {field.config.maxSize as number}MB
              </p>
            )}
    ...
    case "heading":
      const HeadingTag = `h${(field.config?.level as number) || 2}` as
        "h1" | "h2" | "h3" | "h4";
      return (
        <HeadingTag className="font-semibold text-gray-800">
          {String(field.config?.text || field.label)}
        </HeadingTag>
      );
        ...
        <div className="space-y-3 p-4 border border-gray-200 rounded-lg bg-gray-50">
          ...
          <div className="space-y-4">
            {items.map((item: any, index: number) => (
              <div
                key={index}
                className="flex gap-4 items-start bg-white p-4 border border-gray-200 rounded-md relative"
              >
                <div className="flex-1 grid grid-cols-1 md:grid-cols-2 gap-4">
                  ...
                  {subFields.length === 0 && (
                    <span className="text-xs text-gray-400">
                      Configure fields in properties
                    </span>
                  )}
                </div>
                {mode !== "canvas" && (
        ...
        <div className="space-y-1">
          ...
          <div className="w-full bg-gray-100 border border-gray-200 rounded-md px-3 py-2 text-sm text-gray-800 font-mono">
            {value !== undefined
              ? String(value)
              : mode === "canvas"
                ? (field.config?.formula as string) || "No formula"
                : "0"}
          </div>
        </div>
      );
