...
function evaluateCondition(cond: Condition, values: Record<string, unknown>): boolean {
  ...
  switch (cond.operator) {
    ...
    case 'not_contains': return Array.isArray(val) ? !val.includes(cond.value) : !String(val || '').includes(String(cond.value))
...
export function FormRenderer({ schema, mode = 'public', onSubmit }: FormRendererProps) {
  ...
  const isMultiStep = schema.settings?.isMultiStep && schema.sections.length > 1 && mode !== 'canvas'

  // Evaluate Logic Rules
  const fieldOverrides = useMemo(() => {
    const overrides: Record<string, { hidden?: boolean; disabled?: boolean; required?: boolean; value?: unknown }> = {}
    
    if (!schema.logic) return overrides

    for (const rule of schema.logic) {
      const conditionResults = rule.conditions.map(cond => evaluateCondition(cond, values))
      const isMatch = rule.conditionOperator === 'ANY' 
      ...
      if (isMatch) {
        for (const action of rule.actions) {
          ...
          if (action.type === 'SET_VALUE' && action.value !== undefined) target.value = action.value
        }
      }
    }
    return overrides
  ...
  const validateSection = (section: FormSection): boolean => {
    ...
    for (const field of section.fields) {
      ...
      const finalRequired = fieldOverrides[field.id]?.required ?? field.required
      if (!finalHidden && finalRequired && (values[field.id] === undefined || values[field.id] === '' || (Array.isArray(values[field.id]) && (values[field.id] as any[]).length === 0))) {
  ...
  const activeSections = isMultiStep ? [schema.sections[currentStep]] : schema.sections
    ...
    <form onSubmit={handleSubmit} className="space-y-8">
      {schema.title && mode !== 'canvas' && (
        <div className="text-center mb-8">
          ...
          {schema.description && <p className="mt-2 text-gray-500">{schema.description}</p>}
        </div>
      )}
      ...
      {isMultiStep && (
        <div className="mb-8">
          <div className="flex items-center justify-between text-sm font-medium text-gray-500 mb-2">
            ...
            <span>{Math.round(((currentStep + 1) / schema.sections.length) * 100)}%</span>
          </div>
          <div className="w-full bg-gray-200 rounded-full h-2">
            <div className="bg-blue-600 h-2 rounded-full transition-all duration-300" style={{ width: `${((currentStep + 1) / schema.sections.length) * 100}%` }}></div>
          </div>
        </div>
      )}
...
function SectionRenderer({ section, values, errors, onChange, mode, fieldOverrides }: {
  section: FormSection
  ...
  return (
    <div className="space-y-6">
      ...
      <div className={`grid gap-6 ${section.columns === 2 ? 'grid-cols-2' : section.columns === 3 ? 'grid-cols-3' : 'grid-cols-1'}`}>
        {section.fields.map(field => {
          ...
          const val = overrides.value !== undefined ? overrides.value : values[field.id]

          return (
            <div key={field.id} className={field.layout?.width === 'half' ? 'col-span-1' : field.layout?.width === 'third' ? 'col-span-1' : 'col-span-full'}>
              <FieldRenderer
