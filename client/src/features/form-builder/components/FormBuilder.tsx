...
export function FormBuilder({ form, initialSchema, onSave, onWorkflowAction, isWorkflowLoading }: FormBuilderProps) {
  ...
  const handleDragEnd = (e: any) => {
    ...
    if (activeData.type === 'LIBRARY_FIELD') {
      const sectionId = overData.type === 'SECTION' ? overData.sectionId : overData.sectionId
      if (sectionId) {
        builder.addField(sectionId, activeData.fieldType)
      }
    } else if (activeData.type === 'CANVAS_FIELD' && overData.type === 'CANVAS_FIELD') {
      const fromSection = activeData.sectionId
      const toSection = overData.sectionId
      if (fromSection === toSection) {
        const section = builder.schema.sections.find(s => s.id === fromSection)
        if (section) {
          const oldIndex = section.fields.findIndex(f => f.id === activeData.fieldId)
          const newIndex = section.fields.findIndex(f => f.id === overData.fieldId)
          ...
          <PropertiesPanel
            ...
            onUpdate={(id, updates) => {
              if (builder.selectedFieldSectionId) {
                builder.updateField(builder.selectedFieldSectionId, id, updates)
              }
            }}
          />
        </div>
