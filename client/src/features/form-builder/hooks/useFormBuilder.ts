...
function builderReducer(state: BuilderState, action: Action): BuilderState {
  switch (action.type) {
          ...
          ? {
              ...
              fields:
                action.index !== undefined
                  ? [
                      ...s.fields.slice(0, action.index),
                      newField,
                      ...s.fields.slice(action.index),
                    ]
                  : [...s.fields, newField],
            }
          : s,
    ...
    case "REMOVE_SECTION": {
      const sections = state.schema.sections.filter(
    ...
    case "UNDO": {
      ...
      return {
        ...state,
        schema: state.history[historyIndex],
        historyIndex,
        isDirty: true,
      };
    }
...
export function useFormBuilder(initialSchema?: FormSchema) {
  ...
  const addField = useCallback(
    (sectionId: string, fieldType: string, index?: number) =>
  ...
  const selectField = useCallback(
    (fieldId: string | null) => dispatch({ type: "SELECT_FIELD", fieldId }),
    [],
  );
  ...
  const updateSchemaSettings = useCallback(
  ...
  const setSaving = useCallback(
    (isSaving: boolean) => dispatch({ type: "SET_SAVING", isSaving }),
    [],
  );
